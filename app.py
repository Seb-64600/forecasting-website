...
url_predict = 'https://forecasting-gold-price-641911840515.europe-west1.run.app/predict'
url_get_gold = 'https://forecasting-gold-price-641911840515.europe-west1.run.app/get_gold'
...
params_predict = {"end_date" : selected_date.strftime('%Y-%m-%d')}

# param days à changer à notre guise si on veut afficher plus de profondeur de valeurs réelles past
params_get_gold = {"end_date" : (selected_date+timedelta(1)).strftime('%Y-%m-%d')}
...
with col1:
    st.markdown(f"<div class='card'><h3>Prévision du prix de cloture de l'or au {selected_date}</h3><h1>{predicted_value} $</h1></div>",
